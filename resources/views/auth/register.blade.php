...
@section('content')
        ...
        <form method="POST" action="{{ route('register.handle') }}">
            ...
            <!-- Patronymic -->
            <div class="mt-4">
                <label for="patronymic" class="text-secondary">Отчесество:</label>
            ...
            <div class="mt-4">
                @captcha <label for="password" class="text-secondary">Введите код с картинки:</label>
            <input type="text" id="captcha" name="captcha" autocomplete="off" class="form-control" />
            </div>

            <div class="flex items-center justify-end mt-4">
                <a class="link-info" href="{{ route('login') }}">
                    {{ __('Already registered?') }}
                </a>

                <button type="submit" class="btn btn-outline-info ms-5">
                    {{ __('Register') }}
                </button>
            </div>
            </div>
